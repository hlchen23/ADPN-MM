...
def eval_test(model, data_loader, device, mode='test', epoch=None, global_step=None, configs=None):
    ...
    with torch.no_grad():
        for idx, (records, vfeats, vfeat_lens, afeats, afeat_lens, word_ids, char_ids, s_labels, e_labels, *_) in tqdm(
            ...
            start_logits_av, end_logits_av, start_logits_v, end_logits_v, _ = model(word_ids, char_ids, vfeats, video_mask, afeats, audio_mask, query_mask)
            
            start_logits = start_logits_av
            end_logits = end_logits_av
            
            rho, s_av, s_v = grader(start_logits_av, end_logits_av, \
            ...
            start_indices_v, end_indices_v = model.extract_index(start_logits_v, end_logits_v)
            start_indices_v = start_indices_v.cpu().numpy()
            end_indices_v = end_indices_v.cpu().numpy()
            rho, s_av, s_v = rho.cpu().numpy(), s_av.cpu().numpy(), s_v.cpu().numpy()
            
            for record, start_index, end_index, start_index_v, end_index_v, rho_i, s_av_i, s_v_i in zip(records, start_indices, end_indices, start_indices_v, end_indices_v, rho, s_av, s_v):
                ...
                start_time_v, end_time_v = index_to_time(start_index_v, end_index_v, record["v_len"], record["duration"])
                iou_v = calculate_iou(i0=[start_time_v, end_time_v], i1=[record["s_time"], record["e_time"]])
                
                ious.append(iou)
                rec.append(record.copy())
                
                rec[-1]['v_pre_s'] = start_time_v
                rec[-1]['v_pre_e'] = end_time_v
                rec[-1]['av_pre_s'] = start_time
                rec[-1]['av_pre_e'] = end_time
                
                rec[-1]['rho'], rec[-1]['s_av'], rec[-1]['s_v'] = rho_i, s_av_i, s_v_i
